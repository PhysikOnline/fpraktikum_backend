...
class CheckPartnerView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not partner_data:
            err_data = {"error": "Dieser User existiert nicht im Elearning System."} # TODO return http 200 and empty dict
            return Response(data=err_data, status=status.HTTP_400_BAD_REQUEST)
        ...
        if check["status"]:
            ...
            return Response(data=resp_data,status=status.HTTP_400_BAD_REQUEST)       # TODO http 200
